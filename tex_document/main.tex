...
\begin{document}
	...
	\subsection{Other notations and generalities on least squares}
	 ...
	 \subsection{Reminders of nonlinear programming}
	 ...
	 \begin{equation}
	 	\label{eq:al}
	 	\calL(x,\lambda,\mu) := \dfrac{1}{2}\|r(x)\|^2 + \scal{\lambda}{h(x)} + \dfrac{\mu}{2} \|h(x)\|^2,
	 \end{equation}
	 ...
	 \begin{equation}
	 	\label{eq:al_grad}
	 	\nabla_x \calL(x,\lambda,\mu) = J(x)^Tr(x) + A^T\pi(x,\lambda,\mu),
	 \end{equation}
	 ...
	 \begin{equation}\label{eq:al_hessian}
	 	\nabla^2_{xx} \calL(x,\lambda,\mu) = J(x)^TJ(x) + \mu A(x)^TA(x) +  S(x) + \sum_{i=1}^d \nabla^2 h_i(x) \pi(x,\lambda,\mu).
	 \end{equation}
	 ...
	 where $H_k:=\nabla^2_{xx} \calL(x_k,\lambda_k,\mu_k)$ or an approximation of it and $g_k:=\nabla_x \calL(x_k,\lambda_k,\mu_k)$.
